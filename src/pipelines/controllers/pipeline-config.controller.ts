...
import { UpsertPipelineDto, UpdateGlobalConfigDto } from '../dto/pipeline.dto.js';
import { PipelineConfigService } from '../services/pipeline-config.service.js';

@Controller('pipelines')
export class PipelineConfigController {
  public constructor(private readonly pipelineConfigService: PipelineConfigService) {}
  ...
  @Put('config')
  public async updateGlobalConfig(@Body() dto: UpdateGlobalConfigDto): Promise<GlobalPipelinesConfig> {
    return this.pipelineConfigService.saveGlobalConfig({ defaults: dto.defaults });
  }
  ...
  @Put(':id')
  public async update(@Param('id') id: string, @Body() dto: UpsertPipelineDto): Promise<PipelineConfig> {
