export default () => ({
  ...
  jetstreamPublishTimeoutMs: Number.parseInt(process.env.JETSTREAM_PUBLISH_TIMEOUT_MS ?? '5000', 10),
  logLevels: (process.env.LOG_LEVEL ?? 'log,error,warn').split(',').map((value) => value.trim()).filter(Boolean),
});
